...
Every generated **Markdown** file MUST include this frontmatter block as the very first thing in the file, before any other content.

`Architecture/System Map.canvas` and `Documentation.base` are exempt: JSON Canvas and Bases files cannot carry Markdown frontmatter, and adding it would make them unopenable. `Index.md` carries only the five fields its template defines, since the module-specific fields do not apply to a hub page. Phase 3 verification must apply this same exemption list, or it fails on every run.
...
**`mode`**
Either `quick` or `full`. This records which run produced the file; it does **not** vary the depth of a module doc. Both modes produce all three audience levels (see §2) — `full` adds whole directories (`Patterns/`, `Onboarding/`, `Cross-Cutting/`), not extra depth per module.
...
xychart-beta
    ...
    y-axis "Count" 0 --> <set this to the highest category count, rounded up — a hard-coded 10 makes bars overflow the axis on any project with more than ten issues in a category>
    bar [4, 3, 7]
```
...
**`Documentation.base`** (native, no plugins) — an Obsidian Bases catalog with interactive table/card views, filterable by type, complexity, language. See `output-structure.md` for the `.base` schema, which is **YAML**, not JSON, and whose filters must use the recursive `and`/`or`/`not` form rather than a flat array.
