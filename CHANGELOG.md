...
- **A regeneration gate must cover every input its output consumes**, checked against the output's row in the Phase 2 dispatch table. The first version of the System Overview gate did not: it keyed on the dependency graph and module purposes but omitted **system-wide patterns**, which are also one of its stated inputs and which shift whenever a module is re-analyzed. Synthesis rewrites that section every run, so the gate would have let the vault's flagship document silently stop matching the synthesis it projects. `architecture_type` and `system_patterns` are now persisted in `analysis.json` — so the comparison is possible without reading the previous `synthesis.md` back — and `patterns_changed` joins the gate. `issues_changed` was widened for the same reason: the Health writers read §7 prose, which Pass 2 rewrites for any re-analyzed module even when the issue records are unchanged.
- **`Documentation.base` is regenerated on a module-set or purpose change.** It was a Phase 2 output with no row in the update flow at all, so the Bases catalog silently kept listing the old module set after an add or remove.
- **Synthesis is dispatched to an agent in both branches** — Sonnet for ≤4 tree-shaped modules, Opus above that — instead of the small-codebase case being done inline by the orchestrator. Inline meant Sonnet-level narrative writing billed at Opus, which is the exact mistake the dispatch tables exist to prevent and which contradicted the skill's own Rationalization Trap.
- **Deleted modules' issues are marked `resolved` in the Step 6 merge**, where issue merging happens, rather than in Step 7. Step 6 writes the array Step 8 persists, so a rule sitting in Step 7 was both out of order and absent from the merge table.
- **Carry-forward damage detection covers four cases, not one** — a mismatched `source-commit`, a missing report, one with no frontmatter, and one missing a required `###` heading all mark the module affected. Previously only a mismatch was caught, so a crashed write or partial checkout left the vault carrying forward a path that did not exist.
- **The v1 migration now names the agent that writes §7.** The Pass 1 prompt forbids writing that section, so backfilled reports would have stopped at §6 — failing the seven-heading completeness check and looking damaged to the next update. A second Haiku agent reformats the issue records v1 already persisted. Migrated §7 has no before/after snippets (v1 stored records, not prose), and the skill now says so rather than letting a thin Health entry read as a clean module.
- **Cross-module regeneration is gated on structural signals** (`graph_changed`, `issues_changed`, `purposes_changed`, `patterns_changed`, `modules_changed`) rather than running unconditionally. These outputs are projections of the dependency graph, the module purposes, and the issue set; if none moved, regenerating produced near-identical prose at Sonnet cost and churned the vault diff. The common update — a bug fix touching no imports — now correctly skips System Overview, Dependency Map, and System Map. Skips must be reported with the unchanged signal named, since a silent skip is indistinguishable from a bug.
